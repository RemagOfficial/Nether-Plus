...
import net.mcreator.netherplus.block.CrimsonStemBlock;
import net.mcreator.netherplus.block.CrimsonNyliumBlock;
...
public class CrimsonForestBiome extends NetherplusModElements.ModElement {
	@ObjectHolder("netherplus:crimson_forest")
	public static final CustomBiome biome = null;
	public CrimsonForestBiome(NetherplusModElements instance) {
		super(instance, 12);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(2f).precipitation(Biome.RainType.NONE)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(CrimsonNyliumBlock.block.getDefaultState(),
							Blocks.NETHERRACK.getDefaultState(), Blocks.NETHERRACK.getDefaultState())));
			setRegistryName("crimson_forest");
			DefaultBiomeFeatures.addCarvers(this);
			DefaultBiomeFeatures.addStructures(this);
			DefaultBiomeFeatures.addMonsterRooms(this);
			DefaultBiomeFeatures.addOres(this);
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(CrimsonStemBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(Blocks.NETHER_WART_BLOCK.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == CrimsonNyliumBlock.block.getDefaultState().getBlock()
							|| ground == Blocks.NETHERRACK.getDefaultState().getBlock())
							&& (ground2 == CrimsonNyliumBlock.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
											...
											setTreeBlockState(changedBlocks, world, blockpos, Blocks.NETHER_WART_BLOCK.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, CrimsonStemBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == CrimsonStemBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.NETHER_WART_BLOCK.getDefaultState().getBlock()
					|| blockType == CrimsonNyliumBlock.block.getDefaultState().getBlock()
