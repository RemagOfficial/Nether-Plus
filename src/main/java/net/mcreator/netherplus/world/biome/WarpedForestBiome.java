...
package net.mcreator.netherplus.world.biome;
...
import net.mcreator.netherplus.block.WarpedWartBlockBlock;
import net.mcreator.netherplus.block.WarpedStemBlock;
import net.mcreator.netherplus.block.WarpedNyliumBlock;
...
public class WarpedForestBiome extends NetherplusModElements.ModElement {
	@ObjectHolder("netherplus:warped_forest")
	public static final CustomBiome biome = null;
	public WarpedForestBiome(NetherplusModElements instance) {
		super(instance, 11);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(2f).precipitation(Biome.RainType.NONE)
					.category(Biome.Category.NETHER).waterColor(4159204).waterFogColor(329011).parent("nether")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(WarpedNyliumBlock.block.getDefaultState(),
							Blocks.NETHERRACK.getDefaultState(), Blocks.NETHERRACK.getDefaultState())));
			setRegistryName("warped_forest");
			DefaultBiomeFeatures.addCarvers(this);
			DefaultBiomeFeatures.addStructures(this);
			DefaultBiomeFeatures.addMonsterRooms(this);
			DefaultBiomeFeatures.addOres(this);
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(WarpedStemBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(WarpedWartBlockBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == WarpedNyliumBlock.block.getDefaultState().getBlock() || ground == Blocks.NETHERRACK.getDefaultState().getBlock())
							&& (ground2 == WarpedNyliumBlock.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == WarpedWartBlockBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, WarpedWartBlockBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, WarpedStemBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == WarpedStemBlock.block.getDefaultState().getBlock()
					|| blockType == WarpedWartBlockBlock.block.getDefaultState().getBlock()
					|| blockType == WarpedNyliumBlock.block.getDefaultState().getBlock()
